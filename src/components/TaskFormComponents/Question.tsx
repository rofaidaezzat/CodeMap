...
const Question: React.FC<IProps> = ({ 
  ...
  const handleOptionChange = (questionId: string, optionText: string) => {
    if (!questionId || !optionText) {
      console.warn('Invalid questionId or optionText:', { questionId, optionText });
      return;
    }
  ...
  if (!questionId || !currentQuestion.options || !Array.isArray(currentQuestion.options)) {
  ...
  const selectedAnswer = userAnswer?.answers?.[0] || '';
