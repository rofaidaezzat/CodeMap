...
import { useGetTasksQuery, useSubmitQuizMutation } from "@/app/services/crudTasks";
...
const TaskForm: React.FC = () => {
    ...
    const { CurrentTaskId } = useSelector((state: RootState) => state.CurrentTaskId);
    ...
    // RTK Query Hooks
    const { data, isLoading, error, refetch } = useGetTasksQuery(CurrentTaskId ?? skipToken);
    ...
    const isFormValid = data?.questions?.every((question: QuestionType) => {
        const questionId = question.questionId || question._id;
        const userAnswer = userAnswers.find(answer => answer.questionId === questionId);
        return userAnswer && userAnswer.answers.length > 0;
    }) ?? false;

    // Progress calculation
    const answeredQuestionsCount = data?.questions?.filter((question: QuestionType) => {
        const questionId = question.questionId || question._id;
        const userAnswer = userAnswers.find(answer => answer.questionId === questionId);
        return userAnswer && userAnswer.answers.length > 0;
    }).length ?? 0;
    ...
    const handleAnswerChange = useCallback((questionId: string, selectedAnswer: string) => {
        ...
        setUserAnswers(prevAnswers => {
            ...
            if (existingAnswerIndex !== -1) {
                ...
                newAnswers[existingAnswerIndex] = { questionId, answers: [selectedAnswer] };
                return newAnswers;
            } else {
                // Add new answer
                return [...prevAnswers, { questionId, answers: [selectedAnswer] }];
            }
        });
    ...
    useEffect(() => {
        console.log('Modal state changed:', { openModal, quizResult: !!quizResult });
        ...
        return (
            <div className="min-h-screen flex justify-center items-center w-full">
                <div className="text-center p-8 bg-red-50 rounded-lg border border-red-200">
                    <h2 className="text-xl font-semibold text-red-800 mb-4">Failed to Load Quiz</h2>
                    <p className="text-red-600 mb-6">There was an error loading the quiz. Please try again.</p>
                    <ButtonTask onClick={() => refetch()}>
                        Retry
                    </ButtonTask>
                </div>
            </div>
        ...
        return (
            <div className="min-h-screen flex justify-center items-center w-full">
                <div className="text-center p-8 bg-gray-50 rounded-lg">
                    ...
                    <p className="text-gray-600">This quiz doesn't contain any questions.</p>
                </div>
            </div>
        );
    }
                    ...
                    <div className="mt-6">
                        <div className="flex justify-between items-center mb-2">
                            <span className="text-sm font-medium text-blue-100">
                                Progress: {answeredQuestionsCount} of {totalQuestions} questions answered
                            </span>
                            <span className="text-sm font-medium text-blue-100">
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="p-8 space-y-6">
                                ...
                                >
                                    <div className={`text-center p-4 rounded-xl border-2 ${getPerformanceColor(quizResult.percentage).bg} ${getPerformanceColor(quizResult.percentage).border}`}>
                                        <div className={`text-4xl font-bold ${getPerformanceColor(quizResult.percentage).text} mb-1`}>
                                            {quizResult.score}
                                        </div>
                                        <div className="text-sm text-gray-600 font-medium">Final Score</div>
                                        <div className="text-xs text-gray-500 mt-1">out of {quizResult.maxScore || quizResult.totalQuestions}</div>
                                    </div>
                                ...
                                {quizResult.feedback && (
                                    ...
                                    >
                                        ...
                                        <div className="text-blue-700 text-sm italic">{quizResult.feedback}</div>
                                    </motion.div>
                                )}
                            </div>
